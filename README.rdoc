...
[format_json] Change one line format to only JSON in the S3 object. Default is false.
...
[after_flush] A string of scripts, comma separated, that will be run after each log is written to s3. Scripts will be passed the s3 path to the log file in the form "s3://my-s3bucket/path/to/log" as the first argument. Failures will be logged, but not raise an error.

[after_flush_config] A string of config paths, that map to after_flush scripts.
